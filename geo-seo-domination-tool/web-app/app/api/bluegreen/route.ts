...
function composeFileProd(){ return join(root(), 'infra','docker','compose.bluegreen.prod.yml'); }
...
function status(file:string){
  ...
  const ps = sh('docker', ['compose','-f', file, 'ps']);
...
export async function POST(req: NextRequest){
  ...
  const file = prod ? composeFileProd() : composeFile();
  if (!existsSync(file)){
    return NextResponse.json({ ok:false, error:`compose file not found: ${file}` }, { status:400 });
  }

  try{
    if (action === 'status'){
      return NextResponse.json({ ok:true, result: status(file), prod });
    }

    if (action === 'pull'){
      const svc = `web_${target}`;
      const r = sh('docker',['compose','-f', file, 'pull', svc]);
      return NextResponse.json({ ok: r.status===0, result: r, target, prod });
    }

    if (action === 'build'){
      const r = sh('docker',['compose','-f', file, 'build', `web_${target}`]);
      return NextResponse.json({ ok: r.status===0, result: r, target, prod });
    }

    if (action === 'up'){
      const r = sh('docker',['compose','-f', file, 'up','-d', `web_${target}`, 'reverse_proxy']);
      return NextResponse.json({ ok: r.status===0, result: r, target, prod });
    }
    ...
    if (action === 'switch'){
      ...
      return NextResponse.json({ ok: r.status===0, result: r, status: st, target, prod });
    }

    if (action === 'rollback'){
      const current = status(file).current || 'blue';
      const back: Target = current === 'blue' ? 'green' : 'blue';
      const r = await switchTo(back);
      const st = status(file);
      return NextResponse.json({ ok: r.status===0, result: r, status: st, target: back, prod });
    }

    if (action === 'logs'){
      const svc = `web_${target}`;
      const r = sh('docker',['compose','-f', file, 'logs','--tail','200', svc]);
      return NextResponse.json({ ok: r.status===0, result: r, target, prod });
    }
