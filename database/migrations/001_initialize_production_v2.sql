-- =====================================================
-- SUPABASE PRODUCTION DATABASE INITIALIZATION V2
-- =====================================================
-- This migration initializes the complete database schema
-- for the GEO-SEO Domination Tool production environment
--
-- FIXED: Removed IF NOT EXISTS from CREATE POLICY (not supported in PostgreSQL 14)
--
...
-- Drop existing policies if they exist (PostgreSQL 14 compatible)
DROP POLICY IF EXISTS "Users can view own company credentials" ON client_credentials;
DROP POLICY IF EXISTS "Users can insert own company credentials" ON client_credentials;
DROP POLICY IF EXISTS "Users can update own company credentials" ON client_credentials;
DROP POLICY IF EXISTS "Users can delete own company credentials" ON client_credentials;
DROP POLICY IF EXISTS "Anyone can create onboarding sessions" ON saved_onboarding_sessions;
DROP POLICY IF EXISTS "Anyone can read own session" ON saved_onboarding_sessions;
DROP POLICY IF EXISTS "Anyone can update own session" ON saved_onboarding_sessions;
...
CREATE POLICY "Users can insert own company credentials"
  ON client_credentials
  FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Users can update own company credentials"
  ON client_credentials
  FOR UPDATE
  USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can delete own company credentials"
  ON client_credentials
  FOR DELETE
  USING (auth.uid() IS NOT NULL);
