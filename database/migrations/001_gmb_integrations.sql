-- GMB Integrations Table
-- Stores Google My Business OAuth tokens and account information per user/company
...
CREATE INDEX IF NOT EXISTS idx_gmb_integrations_user_id ON gmb_integrations(user_id);
CREATE INDEX IF NOT EXISTS idx_gmb_integrations_company_id ON gmb_integrations(company_id) WHERE company_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_gmb_integrations_organisation_id ON gmb_integrations(organisation_id) WHERE organisation_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_gmb_integrations_status ON gmb_integrations(status);
CREATE INDEX IF NOT EXISTS idx_gmb_integrations_expires ON gmb_integrations(token_expires_at);
