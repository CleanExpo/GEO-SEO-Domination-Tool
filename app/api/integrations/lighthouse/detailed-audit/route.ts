...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get detailed audit
    const audit = await lighthouseService.getDetailedAudit(url, strategy)
    ...
    console.error('Lighthouse detailed audit error:', error)
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to perform detailed Lighthouse audit' },
      { status: 500 }
    )
  }
}
